-- https://github.com/LunarVim/LunarVim/blob/master/lua/lvim/interface/popup.lua

local Popup = {}

--- Create a new floating window
-- @param config The configuration passed to vim.api.nvim_open_win
-- @param win_opts The options registered with vim.api.nvim_set_option_value
-- @param buf_opts The options registered with vim.api.nvim_set_option_value
-- @return A new popup
function Popup:new(opts)
  ...
  local obj = {
    ...
    win_opts = vim.tbl_deep_extend("force", win_opts, opts.win_opts),
    buf_opts = vim.tbl_deep_extend("force", buf_opts, opts.buf_opts),
  }
...
function Popup:display(content_provider)
  ...
  -- key map
  vim.api.nvim_buf_set_keymap(self.bufnr or 0, "n", "q", "<CMD>close<CR>", {
    silent = true,
    desc = "Quit Popup",
  })

  local lines = content_provider(self.layout) or {}
